...
| 1.4 | **Red-team harness v2**: (a) trajectory-length stress mode (AgentDyn: utility →23.6% past 10 steps); (b) "helpful third-party instructions" poisoning family; (c) report utility-and-ASR deltas. **Partially delivered 2026-08-21:** metamorphic M1–M6 property suite + statistical CI layer + ablation study now exist (EVIDENCE.md §1.4–1.5); trajectory-length mode remains open. | **AgentDyn** (Li et al., arXiv:2602.03117): static-benchmark defenses collapse in dynamic environments (Meta SecAlign: 80% utility on AgentDojo → 53.4% on AgentDyn; ASR ×4); **utility drops 100% → 23.6% as trajectory length exceeds 10 steps** → Aegis must be stressed on long chains, not just single calls. **PIArena** (arXiv:2604.08499): effective-utility trade-off quantified via dual Utility+ASR metrics — exactly the dual axis we adopted; multi-step IPI analysis (arXiv:2604.03870) shows single-turn benchmarks hide fragility. |
