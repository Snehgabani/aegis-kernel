	...
	for _, pack := range e.packs {
		for _, rule := range pack.Rules {
			...
			case "sql_ast":
				...
				vList := e.sqlChecker.Evaluate(rule.ID, pack.ID, params, call, rule.Severity)
			...
			case "numeric":
				...
				vList := e.numericChecker.Evaluate(rule.ID, pack.ID, params, call, rule.Severity)
				violations = append(violations, vList...)
			...
			case "regex":
				...
				vList := e.piiChecker.Evaluate(rule.ID, pack.ID, params, call, rule.Severity)
				violations = append(violations, vList...)
			...
			case "state_invariant":
				...
				vList := e.stateChecker.Evaluate(rule.ID, pack.ID, params, call, options.State, rule.Severity)
				violations = append(violations, vList...)
			}
		}
	}
