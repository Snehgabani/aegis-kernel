...
report.sections.tests = {
  ok: tests.ok && !cleanOut.includes('failed'),
  passed: testMatch ? Number(testMatch[1]) : null,
  failed: failedMatch && !cleanOut.includes('passed (') ? Number(failedMatch[1]) : 0,
};
console.log(`[1/6] tests: ${report.sections.tests.passed} passed / ${report.sections.tests.failed} failed`);
...
## 1. Test Suite
${report.sections.tests.failed === 0 ? `✅ **${report.sections.tests.passed} tests passing, 0 failing**` : `❌ ${report.sections.tests.passed} passed / ${report.sections.tests.failed} FAILED`}
