...
# Helper: append a result as JSON to the results file safely via sys.argv
add_result() {
  local category="$1" tool="$2" status="$3" details="$4"
  python3 -c "
import json, sys
results_file, category, tool, status, details, timestamp = sys.argv[1:7]
...
" "${RESULTS_FILE}" "${category}" "${tool}" "${status}" "${details}" "${TIMESTAMP}"
}
